...
function user(e) {
  enemySC.forEach(function (enemysc) {
    if (enemysc >= 95) return;
  });
  if (sc >= 95) return;
  if ((e.key.length > 1 && e.key != "Backspace") || e.key == " ") return;
  if (mode.value == "voc") {
    if (e.key != "Backspace") ans += e.key;
    else ans = ans.slice(0, ans.length - 1);
  } else {
    ans = e.key;
  }
  if (rand == ans) {
    ...
    audio.play();
    if (sc >= 1) sc = sc - 1;
    randalph.style.color = "red";
  } else if (ans == rand.slice(0, ans.length)) randalph.style.color = "black";
  render();
}
...
function enemy(hard) {
  ...
  if (level.value == "auto" && 40 < hard && hard < 100)
    hard = 60 + (sc - (enemySC[0] + enemySC[1] + enemySC[2]) / 3);
  for (let i = 0; i < 3; i++) {
    if (Math.floor(Math.random() * 100) < hard) enemySC[i]++;
    else {
      if (enemySC[i] > 0) enemySC[i]--;
    }
  }
  render();
}
